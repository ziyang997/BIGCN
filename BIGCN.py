...
class LightGCN(object):
    def __init__(self, data_config, pretrain_data):
        ...
        *********************************************************
        ...
        elif self.alg_type in ['bigcn']:
            self.ua_embeddings, self.ia_embeddings = self._create_bigcn_embed()
    ...
    def _init_weights(self):
        ...
        if self.pretrain_data is None:
            ...
            all_weights['K_embedding'] = tf.Variable(initializer([self.emb_dim, self.em_dim]), name='K_embedding')
            all_weights['Q_embedding'] = tf.Variable(initializer([self.emb_dim, self.em_dim]), name='Q_embedding')
    ...
    def _create_bigcn_embed(self):
        ...
        for k in range(0, self.n_layers):
            ego_embeddings_q = tf.matmul(ego_embeddings, self.weight_size['Q_embedding'])
            ego_embeddings_k = tf.matmul(ego_embeddings, self.weight_size['K_embedding'])
